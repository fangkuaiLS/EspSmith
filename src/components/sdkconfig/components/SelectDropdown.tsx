...
export function SelectDropdown({ config, onChange, onResetChildren, canReset }: SelectDropdownProps) {
  const { t } = useTranslation();
  const [showHelp, setShowHelp] = useState(false);

  function handleReset() {
    onResetChildren(config.children.map((child) => child.id));
  }
    ...
    <div className={styles.formGroup}>
      <div className={styles.labelRow}>
        ...
        <div className={styles.iconGroup}>
          ...
          {canReset && (
            <div className={`${styles.infoIcon} ${styles.resetIcon}`} onClick={handleReset} title={t('sdkconfig.resetToDefault')}>
              <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor">
