...
pub fn menu_from_kconfig(project_path: &str, idf_path: &str) -> Result<serde_json::Value, String> {
    let idf_py =
        idf::find_idf_py(idf_path).ok_or_else(|| format!("idf.py not found in {}", idf_path))?;
    ...
    info!(
        "[menu_from_kconfig] Running kconfig_dump.py for {}",
        project_path
    );
    let mut child = start_python_script(
        project_path,
        idf_path,
        &idf_py,
        &script_path,
    ...
    let _ = child.wait();
    if !stderr.is_empty() {
        info!("[menu_from_kconfig] stderr:\n{}", stderr);
    }
    if json_str.trim().is_empty() {
        return Err(format!(
            "kconfig_dump.py produced no output. stderr: {}",
            stderr
        ));
    }
...
fn write_script_temp() -> Result<std::path::PathBuf, String> {
    ...
    let s1 = cwd
        .join("src-tauri")
        .join("scripts")
        .join("kconfig_dump.py");
    let s2 = cwd.join("scripts").join("kconfig_dump.py");
    let dev_script = if s1.exists() { s1 } else { s2 };

    let content = if dev_script.exists() {
        std::fs::read_to_string(&dev_script).map_err(|e| format!("Cannot read source: {}", e))?
    } else {
        KCONFIG_DUMP_PY.to_string()
    };

    std::fs::write(&path, &content).map_err(|e| format!("Cannot write script: {}", e))?;
    Ok(path)
}
...
fn start_python_script(
    ...
    if let Some(eim_setup) = idf::find_eim_setup(idf_path) {
        ...
        cmd.arg(script_path)
            ...
            .env(
                "PATH",
                format!("{}{}{}", py_scripts, idf::PATH_LIST_SEP, sys_path),
            )
            .env(
                "PYTHONPATH",
                format!(
        ...
        cmd.spawn()
            .map_err(|e| format!("Failed to run kconfig_dump.py: {}", e))
    } else {
        if cfg!(windows) {
